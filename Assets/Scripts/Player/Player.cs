...
public class Player : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            float distanceBetweenWaypoints = Vector3.Distance(transform.position, targetPosition);
            percentBetweenWaypoints += Time.deltaTime * moveSpeed / distanceBetweenWaypoints / 2;
            percentBetweenWaypoints = Mathf.Clamp01(percentBetweenWaypoints);
            float easedPercentBetweenWaypoints = Ease(percentBetweenWaypoints, 1f);

            Vector3 newPos = Vector3.Lerp(transform.position, targetPosition, easedPercentBetweenWaypoints);
